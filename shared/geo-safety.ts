...
function checkExtremeEnvironments(
  lat: number,
  lng: number
): {
  riskLevel: 'safe' | 'low' | 'medium' | 'high' | 'extreme' | 'forbidden'
...
function checkIfOceanLocation(lat: number, lng: number): boolean {
  ...
  for (const ocean of oceanAreas) {
    if (
      lat <= ocean.bounds.north &&
      lat >= ocean.bounds.south &&
      lng <= ocean.bounds.east &&
      lng >= ocean.bounds.west
    ) {
    ...
    for (const keyword of keywords) {
      if (lowerContent.includes(keyword.toLowerCase())) {
        ...
        if (category === 'ILLEGAL_ACTIVITIES' || category === 'VIOLENCE') {
          ...
          // For fraud, only set to 'medium' if we don't already have a higher severity
